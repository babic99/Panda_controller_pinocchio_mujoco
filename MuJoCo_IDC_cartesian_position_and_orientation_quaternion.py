...
def rot_to_quat_xyzw(R):
    q = pin.Quaternion(R)
    q.normalize()
    return q.coeffs().copy()  # [x, y, z, w]


def quat_xyzw_to_rot(q_xyzw):
    q_xyzw = quat_normalize(q_xyzw)
    q = pin.Quaternion(q_xyzw[3], q_xyzw[0], q_xyzw[1], q_xyzw[2])  # (w, x, y, z)
    q.normalize()
...
def quat_inverse_xyzw(q):
    q = quat_normalize(q)
    return quat_conjugate_xyzw(q)


def quat_multiply_xyzw(q1, q2):
    """
    Množenje kvaterniona u formatu [x, y, z, w]
    """
    x1, y1, z1, w1 = q1
    x2, y2, z2, w2 = q2
...
def quat_slerp_xyzw(q0, q1, s):
    """
    SLERP za kvaternione u formatu [x, y, z, w]
    """
    ...
    w0 = np.sin(theta_0 - theta) / sin_theta_0
    w1 = sin_theta / sin_theta_0

    q = w0 * q0 + w1 * q1
    return quat_normalize(q)


def quat_error_local_vector_xyzw(q_act_xyzw, q_des_xyzw):
    """
    Kvaternion greške u LOKALNOM okviru:
        q_err = q_act^{-1} * q_des

    Iz toga pravimo 3D rotacioni vektor:
        rot_err_local = axis * angle

    To je kvaternionski ekvivalent:
        log3(R_act.T @ R_d)
    """
    q_act_xyzw = quat_normalize(q_act_xyzw)
    q_des_xyzw = quat_normalize(q_des_xyzw)

    q_err = quat_multiply_xyzw(quat_inverse_xyzw(q_act_xyzw), q_des_xyzw)
    ...
    rot_err_local = axis_local * angle
    return rot_err_local
...
axis_world_B = np.array([0.0, 0.0, 0.0], dtype=float)
theta_B = np.deg2rad(0.0)

R_B_des = pin.exp3(axis_world_B * theta_B) @ R_A_des

# kvaternioni targeta
q_A_des = rot_to_quat_xyzw(R_A_des)
q_B_des = rot_to_quat_xyzw(R_B_des)
...
with viewer.launch_passive(model_mj, data_mj) as v:
    while v.is_running():
        ...
        # -----------------------------------
        # (C2) Orijentacija preko kvaterniona (SLERP)
        # -----------------------------------
        sigma, _, _ = quintic_phase(T_segment, t_local)

        q_d_xyzw = quat_slerp_xyzw(q_start_des, q_goal_des, sigma)
        R_d = quat_xyzw_to_rot(q_d_xyzw)

        # numericka aproksimacija zeljene ugaone brzine i ubrzanja
        eps_t = 1e-4

        sigma_p, _, _ = quintic_phase(T_segment, np.clip(t_local + eps_t, 0.0, T_segment))
        sigma_m, _, _ = quintic_phase(T_segment, np.clip(t_local - eps_t, 0.0, T_segment))

        q_p = quat_slerp_xyzw(q_start_des, q_goal_des, sigma_p)
        q_m = quat_slerp_xyzw(q_start_des, q_goal_des, sigma_m)

        R_p = quat_xyzw_to_rot(q_p)
        R_m = quat_xyzw_to_rot(q_m)

        rotvec_pm = pin.log3(R_m.T @ R_p)
        omega_d = R_d @ (rotvec_pm / (2.0 * eps_t))

        rotvec_dp = pin.log3(R_d.T @ R_p)
        rotvec_md = pin.log3(R_m.T @ R_d)
        domega_d = R_d @ ((rotvec_dp - rotvec_md) / (eps_t ** 2))
        ...
        q_act_xyzw = rot_to_quat_xyzw(R_act)
        ...
        # orijentacija - greska preko kvaterniona
        rot_err_local = quat_error_local_vector_xyzw(q_act_xyzw, q_d_xyzw)
        ...
        dq_ref_full[:7] = np.clip(dq_ref_full[:7], -0.5, 0.5)
        dq_ref_full[7:] = 0.0
